...
def toRgb(inColor):
    ...
    for i in range(0, len(opt)):
        strs += str(round(int(opt[i], 16)/255,2)) + ","
    strs = strs + "0.2"
    return strs
...
class detPar:
    ...
    def __init__(self, config, n, path):
        ...
        if(self.shape == "G4Box"):
            ...
            #self.size.append(config.get("detector", "BodySizeA"+n))
            #self.size.append(config.get("detector", "BodySizeA"+n))
            self.size = config.get("detector", "BodySize"+n).split(',')
        ...
        self.pos = config.get("detector", "BodyPos"+n).split(',')
        ...
        self.rot = config.get("detector", "BodyRot"+n).split(',')
    ...
    def genDetSrcVol(self):
        ...
        if(self.mothID == "-1"):
            genstr = genstr.replace(
                'fDetPar['+self.id+']->Region', '//fDetPar['+self.id+']->Region')
...
class genGun:
    ...
    def __init__(self, config, path):
        self.type = config.get("gun", "GunType")
        if(self.type == 'Mono'):
            Particle = config.get("gun", "GunParticle")
            Energy = config.get("gun", "GunEnergy")
            Position = config.get("gun", "GunPosition")
            Direction = config.get("gun", "GunDirection")
            ...
            self.gunSrcIni = self.gunSrcIni.replace(
                '$gunSrcIni_Particle$', Particle)
            self.gunSrcIni = self.gunSrcIni.replace(
                '$gunSrcIni_Energy$', Energy)
            self.gunSrcIni = self.gunSrcIni.replace(
                '$gunSrcIni_Position$', Position)
            self.gunSrcIni = self.gunSrcIni.replace(
                '$gunSrcIni_Direction$', Direction)
